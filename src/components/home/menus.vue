			...
			<div>
				<el-table :data="tableData" border style="width: 100%" highlight-current-row>
					...
					<el-table-column prop="delFlag" label="删除标识" width="80" align="center">
						<template slot-scope="scope">
							<el-tag :type="scope.row.delFlag === 0 ? 'success' : 'danger'" disable-transitions v-if="scope.row.delFlag === 0 ">使用</el-tag>
							<el-tag :type="scope.row.delFlag === 0 ? 'success' : 'danger'" disable-transitions v-else>删除</el-tag>
						</template>
					</el-table-column>
			...
			<div>
				<el-dialog title="角色详情" :visible.sync="insertDialogFormVisible" :width="fromWidth" center>
					<div class="insert-menu-from">
						<div>
							<el-input v-model="insertFrom.name" placeholder="权限名称"></el-input>
							<el-input v-model="insertFrom.path" placeholder="权限路径"></el-input>
							<el-input v-model="insertFrom.permission" placeholder="权限标识"></el-input>
							<el-input v-model="insertFrom.icon" placeholder="权限图标"></el-input>
						</div>
						<div>
						...
						<div>
							<el-select v-model="insertFrom.parentId" clearable :placeholder="insertFrom.type==1? '请选择上级目录':'请选择上级菜单'">
								<el-option v-for="item in fatherMenu" :key="item.menuId" :label="item.name" :value="item.menuId">
								</el-option>
							</el-select>
							<span>{{insertFrom.parentId}}</span>
						</div>
					</div>
